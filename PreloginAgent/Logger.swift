...
private func file() -> WritingHandle {
    let path = "/var/log/login-sandbox-prelogin-agent.log"
    if !FileManager.default.fileExists(atPath: path) {
        print("Creating log file at \(path)")
        if FileManager.default.createFile(atPath: path, contents: nil) == false {
...
private let queue = DispatchQueue(label: "ca.burea.labs.login-sandbox.prelogin-agent.logger")
